...
public class freserva {
    ...
    String [] titulos = {"ID","Idhabitacion","Numero","idcliente","Cliente","idtrabajador","Trabajador","Tipo Reserva","Fecha Reserva","Fecha Ingreso","Fecha Salida","Costo","Estado"};
       ...
       sSQL="select r.idreserva,r.idhabitacion,h.numero,r.idcliente,"+
               ...
               "r.tiporeserva,r.fecha_reserva,"+
               "r.fecha_ingreso,r.fecha_salida, r.costo_alojamiento,r.estado from reserva r inner join habitacion h on r.idhabitacion=h.idhabitacion where r.fecha_reserva like '%"+ buscar +"%' order by idreserva desc";
    ...
    public boolean insertar(vreserva dts) {
        sSQL = "insert into reserva (idhabitacion,idcliente,idtrabajador,tiporeserva,fecha_reserva,fecha_ingreso,fecha_salida,costo_alojamiento,estado)"
                + "VALUES(?,?,?,?,?,?,?,?,?)";
        try {
    ...
    public boolean editar(vreserva dts) {
        sSQL = "UPDATE reserva set idhabitacion=?,idcliente=?, idtrabajador=?,tiporeserva=?,fecha_reserva=?,fecha_ingreso=?,fecha_salida=?,costo_alojamiento=?,estado=?"
                + "where idreserva=?";
